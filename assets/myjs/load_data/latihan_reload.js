...
function load_item(id){
    ...
    let result = ajax(url_base+"program/get_all_latihan_pertemuan", "POST", data);
    ...
    if(result.item.length != 0) {
        result.item.forEach(data => {
            if(data.item == "soal"){
                ...
                item = `<center><audio controls controlsList="nodownload"><source src="`+url_base+`assets/media/`+data.data+`?t=`+Math.random()+`" type='audio/mpeg'></audio></center>`
